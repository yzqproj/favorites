...
//	private RedisService redisService;
    ...
    public void autoRecovery() {
		...
		Timestamp date = new Timestamp(ca.getTime().getTime());
		...
		for(Collect collect : collectList){
			try {
				...
				collectRespository.modifyIsDeleteById(IsDelete.YES, new Timestamp(System.currentTimeMillis()), collect.getId());
				favoritesRespository.reduceCountById(collect.getFavoritesId(), new Timestamp(System.currentTimeMillis()));
			} catch (Exception e) {
    ...
    public void getImageLogoUrl(){
		...
		for(UrlLibrary urlLibrary : urlLibraryList){
			try {
				...
				if(result){
					collectRespository.updateLogoUrlByUrl(logoUrl,new Timestamp(System.currentTimeMillis()),urlLibrary.getUrl());
