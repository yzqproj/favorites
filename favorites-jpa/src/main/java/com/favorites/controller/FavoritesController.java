        ...
        if (StringUtils.isNotBlank(favoritesName) && 0 != favoritesId) {
            ...
            if (null != fav && getUserId() == fav.getUserId().longValue()) {
                ...
                if (null != favorites) {
                    ...
                    try {
                        favoritesRepository.updateNameById(favoritesId, new Timestamp(System.currentTimeMillis()), favoritesName);
                    } catch (Exception e) {
        ...
        try {
            ...
            if (null != fav && getUserId() == fav.getUserId().longValue()) {
                ...
                if (null != config) {
                    ...
                    if (null != favorites) {
                        configRespository.updateFavoritesById(config.getId(), String.valueOf(favorites.getId()), new Timestamp(System.currentTimeMillis()));
                    }
                }
            }
