...
public interface CollectRepository extends JpaRepository<Collect, Long> {
    ...
    List<Collect> findByCreateTimeLessThanAndIsDeleteAndFavoritesIdIn(Timestamp createTime, IsDelete isDelete, List<Long> favoritesIds);

    @Transactional
    @Modifying
    @Query("update Collect c set c.isDelete = ?1,c.lastModifyTime = ?2 where c.id = ?3")
    int modifyIsDeleteById(IsDelete isDelete, Timestamp lastModifyTime, Long id);

    @Transactional
    @Modifying
    @Query("update Collect c set c.logoUrl = ?1,c.lastModifyTime = ?2 where c.url = ?3")
    int updateLogoUrlByUrl(String logoUrl, Timestamp lastModifyTime, String url);
